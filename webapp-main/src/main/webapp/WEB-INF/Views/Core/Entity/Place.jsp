        ...
        <c:when test="${not empty entity.image and screenWidth != 'EXTRA_SMALL'}">
            <div class="hero" ${markup.property(entity, "image")}>
                ...
                <div class="overlay overlay-tl ribbon">
                    <h1 ${markup.property(entity, "name")}><xpm:property entity="${entity}" property="name"/>${entity.name}</h1>
                </div>
            </div>
        </c:when>
        <c:otherwise>
                ...
                <div class="tile">
                    <div class="vcard">
                        <c:if test="${not empty entity.address}">
                            ...
                            <div class="adr" ${markup.property(entity, "address")}><xpm:property entity="${entity}" property="address"/>
                                ${entity.address}
                            </div>
                        ...
                        <c:if test="${not empty entity.telephone or not empty entity.faxNumber or not empty entity.email}">
                            ...
                            <div class="h-card">
                                <c:if test="${not empty entity.telephone}">
                                    <p>${markup.resource("core.teleponeCaptionText")} <a class="tel" href="tel:${entity.telephone}" ${markup.property(entity, "telephone")}><xpm:property entity="${entity}" property="telephone"/>${entity.telephone}</a></p>
                                </c:if>
                                <c:if test="${not empty entity.telephone}">
                                    <p>${markup.resource("core.faxCaptionText")} <a class="fax" href="fax:${entity.faxNumber}" ${markup.property(entity, "faxNumber")}><xpm:property entity="${entity}" property="faxNumber"/>${entity.faxNumber}</a></p>
                                </c:if>
                                <c:if test="${not empty entity.telephone}">
                                    <p>${markup.resource("core.emailCaptionText")} <a class="email" href="mailto:${entity.email}" ${markup.property(entity, "email")}><xpm:property entity="${entity}" property="email"/>${entity.email}</a></p>
                                </c:if>
                            </div>
                        </c:if>
                    </div>
