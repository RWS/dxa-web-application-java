...
@Component
@Slf4j
public class BrokerDynamicComponentPresentationProvider extends AbstractBrokerComponentPresentationProvider {

    private static final Map<Integer, WebComponentPresentationFactory> FACTORY_CACHE = new ConcurrentHashMap<>();
    private static final String ERROR_MESSAGE
            = "Component Presentation not found for componentId: %d, templateId: %d and publicationId: %d";

    /**
     * {@inheritDoc}
     */
    @Override
    protected ComponentPresentation getDynamicComponentPresentationInternal(int componentId, int templateId,
                                                                            int publicationId)
        ...
        WebComponentPresentationAssembler assembler = new ComponentPresentationAssemblerImpl(publicationId);
        String assembledContent = assembler.getContent(result.getComponentId(), result.getComponentTemplateId());

        return getComponentPresentation(assembledContent);
    }
