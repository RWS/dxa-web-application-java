...
/**
 * //Temporary EHCache implementation
 * <p/>
 * Created by rai on 27/05/14
 * Implemented by Mihai Cadariu on 25.07.2014
 */
public class EHCacheProvider implements CacheProvider, CacheInvalidator {

    /**
     * The name of the EHCache that contains the cached items for this application (Anchorage)
     */
    public static final String CACHE_NAME = "Anchorage";
    private final Cache cache = CacheManager.create().getCache(CACHE_NAME);
    public static final String CACHE_NAME_PRICE = "productPriceInfo";
    private final Cache priceCache = CacheManager.create().getCache(CACHE_NAME_PRICE);
    ...
    private final String TTL_EXPIRED = "cache.expired.ttl"; // keep expired items in cache for another X seconds
    private final int expiredTTL;

    private EHCacheProvider() {
        LOG.debug("Create new instance");
        expiredTTL = 299;
        LOG.debug("Using {} = {} seconds", TTL_EXPIRED, expiredTTL);
    }
    ...
    @Override
    public Object loadFromPriceCache(String cacheKey) {
        if (priceCache == null) {
            LOG.error("Cache configuration is invalid! NOT Caching. Check EH Cache configuration.");
            return null;
        }
        Element element = priceCache.get(cacheKey);
        return element == null ? null : element.getObjectValue();
    }

    @Override
    public void storeInPriceCache(String cacheKey, Object prices) {
        LOG.debug("Store key: {} in cache", cacheKey);
        priceCache.put(new Element(cacheKey, prices));
    }
    ...
    public void invalidate(final String key) {
        ...
        if (element != null) {
            ...
            if (dependentSet != null) {
                for (Iterator<String> iterator = dependentSet.iterator(); iterator.hasNext(); ) {
                    String dependentKey = iterator.next();
