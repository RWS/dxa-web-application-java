...
public abstract class AbstractModelConverter {
	...
	protected <T extends BaseViewModel> void setFieldValue (final T model, final Field f, final org.dd4t.contentmodel.Field renderedField) throws IllegalAccessException {
		...
		if (values != null && !values.isEmpty()) {
			...
			if (renderedField.getFieldType().equals(FieldType.COMPONENTLINK)
					|| renderedField.getFieldType().equals(FieldType.MULTIMEDIALINK)
					|| renderedField.getFieldType().equals(FieldType.EMBEDDED)) {
				// TODO: We should render to a view model here (Or maybe not).
				LOG.debug("Checking");
			}else {
				if (isMultiValued) {
