...
public abstract class BaseFactory {
    ...
    /**
     * Get the filters. If no filters are set the defaults are instantiated and returned.
     */
    ...
    public void doFilters(Item item, BaseFilter.RunPhase runPhase) throws FilterException, SerializationException {
        if (item != null) {
            for (Filter filter : getFilters()) {
                ...
                } else if (runPhase == BaseFilter.RunPhase.BeforeCaching && filter.getCachingAllowed()) {
                    this.doFilter(filter, item);
                } else if (runPhase == BaseFilter.RunPhase.AfterCaching && !filter.getCachingAllowed()) {
                    this.doFilter(filter, item);
                }
            }
        }
    ...
    protected <T extends Item> T deserialize(String source, Class<? extends T> clazz) throws SerializationException {
        T result = SerializerFactory.deserialize(source, clazz);
        return result;
    }
}